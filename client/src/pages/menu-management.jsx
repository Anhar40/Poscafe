    ...
    return (<div className="min-h-screen bg-coffee-50">
          ...
          <Card>
              ...
              <div className="flex justify-between items-center">
                ...
                <Dialog open={showAddCategory} onOpenChange={setShowAddCategory}>
                  ...
                  <DialogContent>
                    ...
                    <CategoryForm category={null} onSubmit={(data) => createCategoryMutation.mutate(data)} isLoading={createCategoryMutation.isPending}/>
                  </DialogContent>
                </Dialog>
              </div>
            </CardHeader>
            <CardContent>
              {categoriesLoading ? (<div className="text-center py-4">
                  ...
                  {categories?.map((category) => (<div key={category.id} className="flex justify-between items-center p-3 bg-coffee-50 rounded-lg">
                      <div>
                        <h3 className="font-medium text-coffee-800">{category.name}</h3>
                        <p className="text-coffee-600 text-sm">{category.description}</p>
                      </div>
                      <div className="flex space-x-2">
                        <Button size="sm" variant="outline" className="border-coffee-200 text-coffee-700" onClick={() => handleEditCategory(category)}>
                          <Edit size={14}/>
                        </Button>
                        <AlertDialog>
                          ...
                          <AlertDialogContent>
                            ...
                            <AlertDialogFooter>
                              ...
                              <AlertDialogAction onClick={() => deleteCategoryMutation.mutate(category.id)} className="bg-red-500 hover:bg-red-600">
                                Hapus
                              </AlertDialogAction>
                            </AlertDialogFooter>
                          </AlertDialogContent>
              ...
              <div className="flex justify-between items-center">
                ...
                <Dialog open={showAddMenuItem} onOpenChange={setShowAddMenuItem}>
                  ...
                  <DialogContent className="max-w-md">
                    ...
                    <MenuItemForm menuItem={null} categories={categories || []} onSubmit={(data) => createMenuItemMutation.mutate(data)} isLoading={createMenuItemMutation.isPending}/>
                  </DialogContent>
                </Dialog>
              </div>
            </CardHeader>
            <CardContent className="max-h-96 overflow-y-auto">
              {menuItemsLoading ? (<div className="text-center py-4">
                  <span className="text-coffee-600">Loading menu...</span>
                </div>) : (<div className="space-y-2">
                  {menuItems?.map((item) => (<div key={item.id} className="flex justify-between items-center p-3 bg-coffee-50 rounded-lg">
                      ...
                      <div className="flex space-x-2">
                        <Button size="sm" variant="outline" className="border-coffee-200 text-coffee-700" onClick={() => handleEditMenuItem(item)}>
                          <Edit size={14}/>
                        </Button>
                        <AlertDialog>
                          <AlertDialogTrigger asChild>
                          ...
                          <AlertDialogContent>
                            ...
                            <AlertDialogFooter>
                              <AlertDialogCancel>Batal</AlertDialogCancel>
                              <AlertDialogAction onClick={() => deleteMenuItemMutation.mutate(item.id)} className="bg-red-500 hover:bg-red-600">
                                Hapus
                              </AlertDialogAction>
                            </AlertDialogFooter>
                          </AlertDialogContent>
      ...
      <Dialog open={showEditCategory} onOpenChange={setShowEditCategory}>
        <DialogContent>
          ...
          <CategoryForm category={editingCategory} onSubmit={(data) => updateCategoryMutation.mutate({ id: editingCategory.id, data })} isLoading={updateCategoryMutation.isPending}/>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={showEditMenuItem} onOpenChange={setShowEditMenuItem}>
        <DialogContent className="max-w-md">
          ...
          <MenuItemForm menuItem={editingMenuItem} categories={categories || []} onSubmit={(data) => updateMenuItemMutation.mutate({ id: editingMenuItem.id, data })} isLoading={updateMenuItemMutation.isPending}/>
        </DialogContent>
      </Dialog>
    </div>);
}
function CategoryForm({ category, onSubmit, isLoading }) {
    ...
    return (<form onSubmit={handleSubmit} className="space-y-4">
      <div>
        ...
        <Input id="categoryName" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder="Contoh: Minuman Kopi" required disabled={isLoading}/>
      </div>
      <div>
        <Label htmlFor="categoryDescription">Deskripsi</Label>
        <Textarea id="categoryDescription" value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} placeholder="Deskripsi kategori (opsional)" disabled={isLoading}/>
      </div>
      <div>
        <Label htmlFor="categoryIcon">Icon</Label>
        <Select value={formData.icon} onValueChange={(value) => setFormData({ ...formData, icon: value })} disabled={isLoading}>
          <SelectTrigger>
      ...
      <div>
        ...
        <Select value={formData.isActive.toString()} onValueChange={(value) => setFormData({ ...formData, isActive: parseInt(value) })} disabled={isLoading}>
          <SelectTrigger>
...
function MenuItemForm({ menuItem, categories, onSubmit, isLoading }) {
    const [formData, setFormData] = useState({
    ...
    return (<form onSubmit={handleSubmit} className="space-y-4">
      <div>
        ...
        <Select value={formData.categoryId} onValueChange={(value) => setFormData({ ...formData, categoryId: value })} disabled={isLoading}>
          <SelectTrigger>
      ...
      <div>
        ...
        <Input id="menuName" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder="Contoh: Cappuccino" required disabled={isLoading}/>
      </div>
      <div>
        <Label htmlFor="menuDescription">Deskripsi</Label>
        <Textarea id="menuDescription" value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} placeholder="Deskripsi menu (opsional)" disabled={isLoading}/>
      </div>
      <div>
        <Label htmlFor="menuPrice">Harga *</Label>
        <Input id="menuPrice" type="number" min="0" step="0.01" value={formData.price} onChange={(e) => setFormData({ ...formData, price: e.target.value })} placeholder="25000" required disabled={isLoading}/>
      </div>
      <div>
        <Label htmlFor="menuImage">URL Gambar (opsional)</Label>
        <Input id="menuImage" type="url" value={formData.imageUrl} onChange={(e) => setFormData({ ...formData, imageUrl: e.target.value })} placeholder="https://example.com/image.jpg" disabled={isLoading}/>
      </div>
      <div>
        <Label htmlFor="menuStatus">Status</Label>
        <Select value={formData.isAvailable.toString()} onValueChange={(value) => setFormData({ ...formData, isAvailable: parseInt(value) })} disabled={isLoading}>
          <SelectTrigger>
            <SelectValue placeholder="Pilih status"/>
          </SelectTrigger>
          <SelectContent>
      ...
      <Button type="submit" disabled={isLoading || !formData.name.trim() || !formData.categoryId || !formData.price} className="w-full bg-coffee-500 hover:bg-coffee-600">
