...
export default function POS() {
    ...
    const handlePaymentComplete = (paymentData) => {
        setReceiptData({
            ...
            cashier: user?.firstName || user?.username || 'User',
            timestamp: new Date(),
        });
    ...
    const { subtotal, tax, total } = calculateTotals();
    return (<div className="min-h-screen bg-coffee-50 overflow-x-hidden">
      {/* Header */}
        ...
        <div className="px-4 sm:px-6 py-3 sm:py-4 flex flex-col sm:flex-row sm:justify-between sm:items-center space-y-2 sm:space-y-0">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-coffee-600 text-xs sm:text-sm">
                Kasir: {user?.firstName || user?.username}
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-2 space-y-2 sm:space-y-0 text-right">
            ...
            <div className="flex space-x-1 sm:space-x-2">
              <Link href="/">
                <Button size="sm" variant="outline" className="border-coffee-200 text-coffee-700 hover:bg-coffee-50 text-xs sm:text-sm">
      ...
      <div className="flex flex-col lg:flex-row h-auto lg:h-[calc(100vh-72px)]">
        ...
        <div className="w-full lg:w-96">
          <OrderPanel currentOrder={currentOrder} orderType={orderType} tableNumber={tableNumber} subtotal={subtotal} tax={tax} total={total} onOrderTypeChange={setOrderType} onTableNumberChange={setTableNumber} onUpdateQuantity={updateQuantity} onRemoveItem={removeFromOrder} onClearOrder={clearOrder} onProcessPayment={processPayment}/>
        </div>
      </div>

      {/* Payment Modal */}
      {showPaymentModal && (<PaymentModal orderData={getOrderData()} onClose={() => setShowPaymentModal(false)} onPaymentComplete={handlePaymentComplete}/>)}

      {/* Receipt Modal */}
      {showReceiptModal && receiptData && (<ReceiptModal receiptData={receiptData} onClose={() => setShowReceiptModal(false)}/>)}
    </div>);
}
